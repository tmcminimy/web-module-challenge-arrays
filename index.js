...
function is31Flavors(){
    if (originalFlavors.length == 31) {    
        console.log("Our shop contains", (originalFlavors.length), "unique flavors. Try em all!");
    }else {
        console.log("We think we have", originalFlavors.length, "? I dunno. We don't pay attention...");
    }
}
is31Flavors();
...
function addFlavor(originalFlavors){
    ...
    console.log("Here's our menu. Check out our new entry at the top!", originalFlavors);
}
...
function removeLastFlavor(originalFlavors){
    originalFlavors.pop();
    console.log("Unfortunately, we are only legally allowed to sell 31 unique things. Our manager scribbled the last one off. Check it out now. Press F to grieve for burnt almond", originalFlavors);
}
...
function getFlavorByIndex(originalFlavors){
    console.log("Our special of the day is half off on any size", originalFlavors[19], ", and you still get a punch on your free ice cream card!")
}
...
function removeFlavorByName(originalFlavors){
    for(var i = 0; i < originalFlavors.length; i++){
        if(originalFlavors[i].includes("Vanilla")){
            originalFlavors.splice(i, 1); i--;        }
        }
...
function biBo(originalFlavors){
    const ogFlavors = [...originalFlavors];
    console.log(ogFlavors);
}
biBo(originalFlavors);
...
function filterByWord(originalFlavors){
    for(var i = 0; i < originalFlavors.length; i++){
        if(originalFlavors[i].includes('Chocolate')){
          console.log("It's World Chocolate Day! See our deals on", originalFlavors[i], "and keep an eye out for when we designate other flavor holidays to sell more ice cream as well!");
        }
    }
}
